package
{
   ...
   public class Game extends Sprite
   {
      ...
      private var sequenceInput:TextField;
      ...
      private function buildOverlayPanel() : void
      {
         ...
         createTextField("Auto-clickers let you send a configured sequence repeatedly.",16,86,OVERLAY_WIDTH - 32,18,labelFormat,this.overlayPanel);
         ...
         sequenceInput = createInputField(180,22,16,162,"Sequence (e.g. 12345)");
         ...
         createLabel("Sequence",16,150,this.overlayPanel);
         createLabel("Interval",212,116,this.overlayPanel);
         createLabel("Hotkey",212,150,this.overlayPanel);
         var addButton:Sprite = createButton("Add Clicker",310,128,90,24,onAddClicker);
         this.overlayPanel.addChild(addButton);
         this.statusText = createTextField("Status: ready.",16,196,OVERLAY_WIDTH - 32,18,labelFormat,this.overlayPanel);
         this.statusText.textColor = 9233407;
         this.clickerListContainer = new Sprite();
         this.clickerListContainer.x = 16;
         this.clickerListContainer.y = 220;
      ...
      private function onInputFocusIn(param1:FocusEvent) : void
      {
         ...
         if(input.text == "Clicker name" || input.text == "Sequence (e.g. 12345)" || input.text == "Interval ms" || input.text == "Press a key")
         {
            input.text = "";
         }
      }
         ...
         if(input.text.length == 0)
         {
            switch(input)
            {
               case this.nameInput:
                  ...
                  break;
               case this.sequenceInput:
                  input.text = "Sequence (e.g. 12345)";
                  break;
      ...
      private function onAddClicker(param1:MouseEvent) : void
      {
         ...
         var sequence:String = this.sequenceInput.text;
         ...
         if(sequence == "" || sequence == "Sequence (e.g. 12345)")
         {
            this.updateStatus("Enter a sequence of characters to send.");
            return;
         }
      ...
      private function renderClickerList() : void
      {
         ...
         while(i < this.clickers.length)
         {
            ...
            var line:String = "[" + clicker.hotkeyName + "] " + clicker.name + " — sequence=\'" + clicker.sequence + "\' interval=" + clicker.interval + "ms " + (clicker.enabled ? "(ON)" : "(OFF)");
      ...
      private function onClickerTimer(param1:TimerEvent) : void
      {
         ...
         this.sendSequence(clicker.sequence);
      }
      
      private function sendSequence(sequence:String) : void
      {
         var j:int = 0;
         while(j < sequence.length)
         {
            var character:String = sequence.charAt(j);
            var code:uint = this.charToKeyCode(character);
            if(code != 0)
            {
               this.dispatchKeyPress(code,character.charCodeAt(0));
            }
            j++;
         }
      }
      ...
      private function dispatchKeyPress(code:uint, charCode:uint = 0) : void
      {
         ...
         var currentFocus:InteractiveObject = this.gameStage.focus;
         this.gameStage.focus = null;
         var down:KeyboardEvent = new KeyboardEvent(KeyboardEvent.KEY_DOWN,true,false,charCode,code);
         var text:TextEvent = new TextEvent(TextEvent.TEXT_INPUT,true,false,String.fromCharCode(charCode));
         var up:KeyboardEvent = new KeyboardEvent(KeyboardEvent.KEY_UP,true,false,charCode,code);
         if(currentFocus)
         {
            currentFocus.dispatchEvent(down);
            currentFocus.dispatchEvent(text);
            currentFocus.dispatchEvent(up);
         }
